...
const getCategories = (query, token) =>
  request.get(
    `/${PLUGIN}/v1/categories?${prepareQuery(query)}`,
    {},
    token,
  );
...
const deleteProduct = (id, token) =>
  request.delete(`/wcfmmp/v1/products/${id}?app-builder-decode=true`, {}, token);

const getOrders = (query, token) =>
  request.get(`/wcfmmp/v1/orders?${prepareQuery(query)}`, {}, token);

const updateOrders = (idOrder, data, token) =>
  request.put(`/wcfmmp/v1/orders/${idOrder}?app-builder-decode=true`, JSON.stringify(data), token);

const getSales = (query, token) =>
  request.get(
    `/wcfmmp/v1/sales-stats?${prepareQuery(query)}`,
    {},
    token,
  );
...
const getStore = (id, token) =>
  request.get(`/wcfmmp/v1/settings/id/${id}?app-builder-decode=true`, {}, token);

const getProfile = (query, token) =>
  request.get(
    `/wcfmmp/v1/user-profile${prepareQuery(query)}`,
    {},
    token,
  );
